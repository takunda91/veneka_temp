...
{
    public partial class CustomerCardSearch : ListPage
    {
        private readonly UserRole[] userRolesForPage = new UserRole[] { UserRole.BRANCH_OPERATOR, UserRole.CENTER_OPERATOR };
        ...
        public int ActivationType
        {
            get
            {
                if (ViewState["ActivationType"] != null)
                {
                    return (int)ViewState["ActivationType"];
                }
                else
                {
                    return 0;
            ...
            set
            {
                ViewState["ActivationType"] = value;
            }
        }
        ...
        protected void LoadPageData()
        {
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(Request.QueryString["ActivationType"]))
                {
                    ActivationType = int.Parse(Request.QueryString["ActivationType"]);
                }
        ...
        private void UpdateProductList(int issuerId)
        {
            ...
            if (issuerId > 0)
            {
                ...
                if (_batchService.GetProductsListValidated(issuerId, CardIssueMethodId, 1, 1000, out products, out messages))
                {
                    ...
                    if (ActivationType == 2)
                    {
                        products = products.Where(p => p.ActivationByCenterOperator == true).ToList();
                    }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ActivationType == 2)
                {
                    cardSearchParams.IsActivationSearch = true;
                }
        ...
        protected override void DisplayResults(ISearchParameters parms, int pageIndex, object[] results)
        {
            ...
            if (results == null)
            {
                var unfiltered = _batchService.SearchCustomerCardsList((CardSearchParameters)parms, PageIndex);
                results = unfiltered.ToArray();
