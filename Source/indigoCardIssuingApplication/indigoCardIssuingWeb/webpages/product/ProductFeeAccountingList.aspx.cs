...
{
    public partial class ProductFeeAccountingList : ListPage
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(ProductFeeAccountingList));
        ...
        private void LoadPageData()
        {
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(Request.QueryString["delete"]))
                {
                    ...
                    if (deleted == 1)
                    {
                        this.lblInfoMessage.Text = "Fee accounting deleted successfully.";
                    }
                }
                ...
                if (issuersList.Count > 0)
                {
                    ...
                    DisplayResults(new ProductAccountingSearchParameters { PageIndex = 1, RowsPerPage = StaticDataContainer.ROWS_PER_PAGE }, 1, null);                    
                }
            }
            catch (Exception ex)
            {
        ...
        protected void dlFeeAccountingList_ItemCommand(Object sender, DataListCommandEventArgs e)
        {
            this.lblErrorMessage.Text = "";
            this.lblInfoMessage.Text = "";
            ...
            try
            {
                dlFeeAccountingList.SelectedIndex = e.Item.ItemIndex;
                string userDetails = ((LinkButton)dlFeeAccountingList.SelectedItem.FindControl("lnkFeeAccountingName")).Text;
                string productstr = ((Label)dlFeeAccountingList.SelectedItem.FindControl("lblFeeAccountingId")).Text;
                ...
                {
                    SessionWrapper.ProductFeeAccountingId = int.Parse(productstr);

                    Server.Transfer("~\\webpages\\product\\ProductFeeAccounting.aspx");
                }
                else
                {
        ...
        {            
            this.lblErrorMessage.Text = "";
            this.dlFeeAccountingList.DataSource = null;
            SearchParameters = parms;

            if (results == null)
            {
                results = _batchservice.GetFeeAccountingList(IssuerId, pageIndex, StaticDataContainer.ROWS_PER_PAGE).ToArray();
            }

            if (results.Length > 0)
            {
                this.dlFeeAccountingList.DataSource = results;
                TotalPages = (int)((ProductFeeAccountingResult)results[0]).TOTAL_PAGES;
            }
            else
            {
                TotalPages = 0;
            ...
            this.dlFeeAccountingList.DataBind();
        }
