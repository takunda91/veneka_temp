
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace Veneka.Indigo.Common.Models
{

using System;
    
public partial class PinBatchResult
{

    public Nullable<int> TOTAL_PAGES { get; set; }

    public Nullable<long> ROW_NO { get; set; }

    public Nullable<int> TOTAL_ROWS { get; set; }

    public long pin_batch_id { get; set; }

    public System.DateTimeOffset date_created { get; set; }

    public string pin_batch_reference { get; set; }

    public int no_cards { get; set; }

    public string status_notes { get; set; }

    public int pin_batch_statuses_id { get; set; }

    public string pin_batch_status_name { get; set; }

    public int issuer_id { get; set; }

    public string issuer_name { get; set; }

    public string issuer_code { get; set; }

    public string card_issue_method_name { get; set; }

    public string branch_name { get; set; }

    public string branch_code { get; set; }

    public Nullable<int> flow_pin_batch_statuses_id { get; set; }

    public Nullable<int> flow_pin_batch_type_id { get; set; }

    public Nullable<int> user_role_id { get; set; }

    public int card_issue_method_id { get; set; }

    public int pin_batch_type_id { get; set; }

    public Nullable<int> reject_pin_batch_statuses_id { get; set; }

}

}
