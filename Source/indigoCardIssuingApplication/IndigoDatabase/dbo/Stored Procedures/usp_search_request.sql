CREATE PROCEDURE [dbo].[usp_search_request] 
	@issuerId int = NULL,
	@product_id int=null,
	@request_reference VARCHAR(50) = NULL,
	@hybrid_request_statuses_id int = NULL,
	@branch_id INT = NULL,
...
BEGIN
    ...
    BEGIN TRANSACTION [GET_CARD_DETAILS_TRAN]
		BEGIN TRY 
		IF(@request_reference = '' or @request_reference IS NULL)
			...
			AS
			(
			SELECT ROW_NUMBER() OVER(ORDER BY request_id ASC, request_reference ASC) AS ROW_NO
					, COUNT(*) OVER() AS TOTAL_ROWS
					, *
			FROM( 
				SELECT distinct 
						hybrid_requests.[request_id], cast (0 as BIGINT) as 'card_id'
					   , CASE @check_masking
							WHEN 1 THEN 
								CASE 
					   ...
					   , [customer_account].[user_id] as 'operator_user_id'
				...
				WHERE 
				 ([hybrid_requests].request_reference like '%'+@request_reference+'%')
					AND hybrid_request_status_current.hybrid_request_statuses_id = COALESCE(@hybrid_request_statuses_id, hybrid_request_status_current.hybrid_request_statuses_id)
					AND [hybrid_requests].card_issue_method_id = COALESCE(@card_issue_method_id, [hybrid_requests].card_issue_method_id)
			...
			ORDER BY request_id ASC, request_reference ASC
